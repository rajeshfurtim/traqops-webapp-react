...
export default function StationWise() {
  ...
  const today = dayjs()
  const firstTypeId = userTypes[0]?.id
  ...
  // Optional: also set filters automatically
  // setFilters(prev => ({
  //   ...prev,
  //   fromDate: today.format('YYYY-MM-DD'),
  //   toDate: today.format('YYYY-MM-DD'),
  //   userTypeId: firstTypeId,
  // }))
 ...
 const reports = useMemo(() => {
  ...
  return (
    <>
      <Helmet>
        <title>{getPageTitle('reports/attendance/station-wise')}</title>
        <meta
          name="description"
