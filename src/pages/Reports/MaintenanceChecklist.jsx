...
export default function MaintenanceChecklist() {
  ...
  const loadReports = async () => {
    try {
      setLoading(true)
      const response = await mockApi.getMaintenanceChecklist(filters)
      setReports(response.data.reports)
    } catch (error) {
      console.error('Error loading maintenance checklist:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <>
      <Helmet>
        <title>{getPageTitle('reports/maintenance-checklist')}</title>
        <meta name="description" content={`${APP_CONFIG.name} - Maintenance Checklist`} />
      </Helmet>
      <Box>
